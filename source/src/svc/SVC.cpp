...
SVC::SVC(const std::string& appIdentity, SVCAuthenticator* authenticator){
	...
	//-- create svc pipe
	uint64_t pipeID;
	crypto::generateRandomData(8, (uint8_t*)&pipeID);
	string svcPipeName = to_string(pipeID);
	this->svcPipe = new NamedPipe(svcPipeName, NamedPipeMode::NP_READ);

	//-- connect to daemon pipe, throwable ERR_NOT_CONNECTED
	this->daemonPipe = new NamedPipe(SVC_DEFAULT_DAEMON_NAME, NamedPipeMode::NP_WRITE);

	//-- create reading thread to read from pipe
	this->incomingQueue = new MutexedQueue<SVCPacket*>();
	this->packetReader = new SVCPacketReader(this->svcPipe, this->incomingQueue);
	...
	packet->pushDataChunk(svcPipeName.c_str(), svcPipeName.size());

	uint8_t buffer[SVC_DEFAULT_BUFSIZ];
	uint16_t packetLen;
	packet->serialize(buffer, &packetLen);
	delete packet;
	if (this->daemonPipe->write(buffer, packetLen) < 0){
		err = ERR_NOT_CONNECTED;
		goto error_cleanup;
	}

	if (!this->packetHandler->waitCommand(SVC_CMD_REGISTER, 0, SVC_DEFAULT_TIMEOUT)){
		err = ERR_TIMEOUT;
		goto error_cleanup;
	}
...
SVCEndpoint* SVC::listenConnection(int timeout, SVCHost* remoteHost){
	// SVCPacket* request;
...
uint64_t SVCEndpoint::getEndpointID(){
...
int SVCEndpoint::sendData(const uint8_t* data, uint16_t dataLen, uint8_t option){
// 	if (this->isAuth){
// 		//-- try to send		
// 		SVCPacket* packet = new SVCPacket(this->endpointID);
...
int SVCEndpoint::readData(uint8_t* data, uint16_t* len, int timeout){
// 	if (this->isAuth){
