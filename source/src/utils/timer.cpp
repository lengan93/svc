...
Timer::Timer(const Timeout &timeout, const void* args)
    : _timeout(timeout),
    _timeoutArgs(args)
{
}

Timer::Timer(const Timer::Timeout &timeout, const void* args,
...
void Timer::setTimeout(const Timeout &timeout, const void* args)
{
    if (this->running() == true)
       return;
